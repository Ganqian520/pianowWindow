...
import './app.scss'
import { Music, Cursor } from './types'
...
export default function App() {
  ...
  let musics: Music[] = [{ indexX: 2, indexY: 49, length: 4 }] //音乐数据
  ...
  function mouseup(e: any) {
    ...
    if (e.button == 0 && cursor == Cursor.miss && !isMoved && x > squareA) {
  ...
  function drawTopGraduation(): void {
    ...
    for (let i = 0; i < count / 4; i++) {
      ctx.beginPath()
      ctx.moveTo(unitW * i * 4, 0)
      ctx.lineTo(unitW * i * 4, squareA)
      ctx.stroke()
      ctx.fillStyle = 'white'
      ctx.textBaseline = 'middle'
      ctx.textAlign = 'center'
      ctx.fillText(`${i + 1}`, unitW * 4 * (i + 0.5), squareA / 2)
    }
    ctx.restore()
  }
  //画左边钢琴条
  function drawLeftPiano(): void {
    ...
    for (let i = 0; i < 88; i++) {
      let str = tf(i)
      //背景
      ctx.fillStyle = str.charAt(0) == '#' ? '#2F4F4F' : 'white'
      ...
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      ctx.fillStyle = str.charAt(0) == '#' ? 'white' : 'black'
